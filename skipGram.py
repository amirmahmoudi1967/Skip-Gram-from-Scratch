...
class SkipGram:
    ...
    def __init__(self,trainset,batch_size=256, w2id=None,wEmbed=None,vocab=None, nEmbed=70, negativeRate=6, winSize = 20, minCounts = 3, epochs = 50, learningRate = 1e-2):
        ...
        if w2id!=None:
            ...
            self.minCounts = minCounts # minimal recurrency of a word to be kept in the vocabulary
            ...
            self.rep=counter
            for word in counter:
                if counter[word] >= self.minCounts:
...
if __name__ == '__main__':
    ...
    if not opts.test:
        ...
        sg= SkipGram(trainset="",w2id=sg["w2id"],wEmbed=sg["wEmbed"],vocab=sg["vocab"])
        for a,b,_ in pairs:
